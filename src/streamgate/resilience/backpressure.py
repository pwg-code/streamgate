...
- kafka 组件断开时指标失效（consumer 不再排空，Kafka 侧积压不可见），
  ...
  （异步缓冲架构主场：DB 堵塞时 age 如实增长，容忍窗内持续接收）
...
class _HysteresisController:
    ...
    def __init__(self, config: BackpressureConfig) -> None:
        ...
        self._rejecting_since: float | None = None  # R4：REJECTING 起始 monotonic 时间戳
    ...
    async def _evaluate(self) -> None:
        """单步判定（测试直接调用；生产由 _run_loop 周期调用）。"""
    ...
    async def _run_loop(self) -> None:
        ...
        R3：REJECTING 态高频探活（unhealthy_check_interval_seconds），
    ...
    def _validate_config(self) -> None:
        """WARN 级校验（不阻断启动）：trip < existence TTL；recover < trip；retries >= 0。"""
...
class HttpProbeSignal(_HysteresisController):
    """周期探活远端 consumer 健康接口的背压信号（双进程拓扑默认）。"""
...
# 兼容别名（原 BackpressureController 的观察者沿用）
BackpressureController = HttpProbeSignal

...
class DefaultErrorClassifier:
    ...
    - asyncio.TimeoutError → RETRY（writer 侧 wait_for 兜底超时等瞬态）
    - aiokafka KafkaError 族 → RETRY（broker 侧瞬态）
    - ValueError → POISON（消费端自身的内容防御：键缺失/naive 时间戳等，
      本质是"消息内容处理不了"）
    - 其余未知异常 → POISON（探针对照兜底，见模块 docstring）
    """
